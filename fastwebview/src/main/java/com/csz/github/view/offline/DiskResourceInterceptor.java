...
public class DiskResourceInterceptor implements Destroyable, ResourceInterceptor {
    ...
    public WebResource load(Chain chain) {
        ...
        if (webResource != null && isRealMimeTypeCacheable(webResource,request)) {
            LogUtils.d(String.format("disk cache hit: %s", request.getUrl()));
            return webResource;
        }
        webResource = chain.process(request);
        if (webResource != null && (webResource.isCacheByOurselves() || isRealMimeTypeCacheable(webResource,request))) {
    ...
    private boolean isRealMimeTypeCacheable(WebResource resource,CacheRequest request) {
        if (resource == null) {
            return false;
        }
